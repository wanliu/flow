...
func (od OrderDeleteConfirm) Confirm(ctx context.Context) (string, interface{}) {
	order, err := database.GetOrderByNo(od.OrderNo)

	if err != nil {
		return fmt.Sprintf("确认操作已经过期, %v 号订单已经不存在", od.OrderNo), nil
	}

	if order.Deletable() {
		err = order.Delete()
		if err == nil {
			return fmt.Sprintf("%v 号订单删除成功", od.OrderNo), nil
		} else {
			return fmt.Sprintf("%v 号订单删除失败，请访问 https://jiejie.io/orders/%v 进行操作", od.OrderNo, order.GlobelId()), nil
		}
	} else {
		return fmt.Sprintf("%v 号订单已经在处理过程中，因此无法删除", od.OrderNo), nil
	}
