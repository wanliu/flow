...
import (
	...
	"github.com/wanliu/brain_data/config"
...
func NewCusOrderResolve(ctx context.Context) *CustomerOrdersResolve {
...
func (r *CustomerOrdersResolve) IsDone() {
	if r.Count != 0 {
		if r.Total >= r.Count {
			return r.Viewed >= r.Count
		} else {
			return r.Viewed >= r.Total
		}
	} else {
		return r.Viewed >= r.Total
	}
...
func (r *CustomerOrdersResolve) Setup(ctx context.Context) {
	if !r.Done {
		ctx.Value(config.CtxKeyCusOrders, r)
	}
}
